...
class fourQueenConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
        ...
        val1 = int(value1)
        val2 = int(value2)
        if value1 != value2 and abs(val1 - val2) != abs(i - j):
            return True
        return False
...
def consistent(assignment, csp, var, value):
    ...
    for variable in csp.binaryConstraints:
        if (variable.affects(var)):
            otherVar = variable.otherVariable(var)
            otherVal = assignment.assignedValues[otherVar]
            if not variable.isSatisfied(value, otherVal):
                return False
    return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    for i in range(0, len(vars)):
        if ((assignment.assignedValues[vars[i]] == None)):
            if ((minValue > len(domains[vars[i]]))):
                varsWithMinValues = []
                minValue = len(domains[vars[i]])
                varsWithMinValues.append(vars[i])
            elif ((minValue == len(domains[vars[i]]))):
                varsWithMinValues.append(vars[i])
    if (len(varsWithMinValues) == 0):
        return None
    elif (len(varsWithMinValues) == 1):
        return varsWithMinValues[0]
    else:
        count = 0
        maxDegree = 0
        nextVar = None
        for var in varsWithMinValues:
            count = 0
            if (len(csp.binaryConstraints) == 0):
                return varsWithMinValues[0]
            for j in csp.binaryConstraints:
                if (j.affects(var)):
                    count = count + 1
            if (maxDegree < count):
                maxDegree = count
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for constraint in csp.binaryConstraints:
        if (constraint.affects(var)):
            affectedVars.append(constraint.otherVariable(var))
    for value in values:
        count = 0
        for var1 in affectedVars:
            if (value in list(assignment.varDomains[var1])):
                count = count + 1
        heuristicVals.append((value, count))
    
    sortedList = sorted(heuristicVals, key = lambda tup: tup[1])
...
def forwardChecking(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if (constraint.affects(var) and assignment.assignedValues[constraint.otherVariable(var)] == None):
            affected.append(constraint.otherVariable(var))
    for variable in affected:
        if (value in list(domains[variable])):
            if (len(domains[variable]) == 1):
                for i in seen:
                    assignment.varDomains[i].add(value)
                return None
            else:
                domains[variable].remove(value)
                inferences.add((variable, value))
    ...
    if (constraint.affects(var1) and constraint.affects(var2)):
        for val2 in assignment.varDomains[var2]:
            ...
            if (count == len(assignment.varDomains[var2])):
                return None
        for tupl in inferences:
            assignment.varDomains[tupl[0]].remove(tupl[1])
        return inferences
    else:
        return inferences
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if (constraint.affects(var) and assignment.assignedValues[constraint.otherVariable(var)] == None):
            tup = (var, constraint.otherVariable(var), constraint)
            que.put(tup)
    while not (que.empty()):
        tuple = que.get()
            ...
            if (len(inference2) != 0):
                for constraint in csp.binaryConstraints:
                    if (constraint.affects(tuple[1]) and assignment.assignedValues[constraint.otherVariable(tuple[1])] == None):
                        tup = (tuple[1], constraint.otherVariable(tuple[1]), constraint)
                        que.put(tup)
...
def AC3(assignment, csp):
    ...
    que = Queue.Queue()
    for variable in csp.varDomains.keys():
        for constraint in csp.binaryConstraints:
            if (constraint.affects(variable)):
                tup = (variable, constraint.otherVariable(variable), constraint)
                que.put(tup)
    while (que.empty() == False):
        deque = que.get()
        revisedInferences = revise(assignment, csp, deque[0], deque[1], deque[2])
        if (revisedInferences == None):
            for infer in inferences:
                assignment.varDomains[infer[0]].add(infer[1])
            return None
        else:
            if (len(revisedInferences) != 0):
                for constraint in csp.binaryConstraints:
                    if (constraint.affects(deque[1]) and assignment.assignedValues[constraint.otherVariable(deque[1])] == None):
                        tup = (deque[1], constraint.otherVariable(deque[1]), constraint)
                        que.put(tup)
                inferences = inferences.union(revisedInferences)
